...
/// AURA Assistant — Gemini Vision Service
///
/// Implements [VisionService] using Google Gemini's native REST API
/// (POST /v1beta/models/{model}:generateContent?key={apiKey})
/// with inline_data for image analysis.
///
/// Mirrors the pattern from [GeminiProvider] (gemini_provider.dart):
///   - Auth: ?key= query parameter (NOT Authorization: Bearer)
///   - Request: contents[] with parts[] containing text + inline_data
///   - Response: candidates[0].content.parts[0].text
///   - API key from 'aura_gemini_api_key' (NOT aura_openai_api_key)
///
/// Preserves [OpenAIVisionService] for OpenAI-compatible providers.
library;
...
import '../../core/ai/ai_connection_config.dart'
    show kDefaultVisionModel, kDefaultBaseUrl;
...
/// Secure storage key for the Gemini API key (shared with GeminiProvider).
const _apiKeyStorageKey = 'aura_gemini_api_key';
...
/// Gemini-native implementation of [VisionService].
///
/// Uses Gemini's generateContent endpoint with inline_data for images,
...
class GeminiVisionService implements VisionService {
  ...
  Future<bool> isAvailable() async {
    final apiKey =
        await connectionStorage.secureStorage.read(key: _apiKeyStorageKey);
...
Respond in JSON format:
{
  ...
  "ocr_text": "any text found in the image"
    ...
    final userText = prompt ??
        'Analyze this image in detail. Identify all objects, people, text, and describe the scene.';

    return _callGeminiVisionAPI(
  ...
  Future<VisionResult> findObject({
    ...
    final systemPrompt =
        '''You are a vision assistant. Find the object described by the user in the image.
...
If the object is NOT found, respond with:
{
  ...
  Future<VisionResult> readText({
    ...
    final langHint =
        language != null ? ' The text may be in $language language.' : '';
    final systemPrompt =
        '''You are an OCR assistant. Read all visible text in the image.$langHint
...
Respond in JSON:
{
  ...
  "targets": [
    ...
    return _callGeminiVisionAPI(
      systemPrompt: systemPrompt,
      userText:
          'Read all visible text in this image. Provide the text content and locations.',
      imageBase64: imageBase64,
    );
  }
...
Respond in JSON:
{
  ...
  "targets": [
    ...
    return _callGeminiVisionAPI(
      systemPrompt: systemPrompt,
      userText:
          'Describe this scene in detail. What kind of place is this? What is happening?',
      imageBase64: imageBase64,
    );
  }
  ...
  Future<VisionResult> locateTarget({
    ...
    final systemPrompt =
        '''You are a target-locating assistant. Find the described target in the image.
...
If the target is NOT found, respond with:
{
  ...
  "targets": []
    ...
    return _callGeminiVisionAPI(
      systemPrompt: systemPrompt,
      userText:
          'Locate "$targetDescription" in this image. Show me exactly where it is.',
      imageBase64: imageBase64,
    );
  }
  ...
  ///
  /// Uses Gemini-native format: POST /models/{model}:generateContent?key={apiKey}
  /// with contents[] containing parts[] with text + inline_data for images.
  Future<VisionResult> _callGeminiVisionAPI({
    ...
    // { contents: [{ role: "user", parts: [{ text: "..." }, { inline_data: { mimeType: "image/jpeg", data: "<base64>" } }] }],
    ...
    try {
        ...
        try {
          if (response.body.isNotEmpty) {
            ...
            errorMessage = error?['message'] as String? ??
                'Gemini Vision API failed with status ${response.statusCode}';
          } else {
            errorMessage =
                'Gemini Vision API failed with status ${response.statusCode} (empty body)';
          }
        } on FormatException {
          errorMessage =
              'Gemini Vision API returned status ${response.statusCode} with non-JSON body';
        } catch (e) {
          errorMessage =
              'Gemini Vision API failed with status ${response.statusCode}';
        }
        return VisionResult.failure(errorMessage);
      }
      ...
      try {
        if (response.body.isEmpty) {
          return VisionResult.failure(
              'Gemini Vision API returned 200 but with empty body');
        }
        data = jsonDecode(response.body) as Map<String, dynamic>;
      } on FormatException {
        return VisionResult.failure(
            'Gemini Vision API returned invalid JSON');
      }
      ...
      if (candidates == null || candidates.isEmpty) {
        ...
        return VisionResult.failure(
            'Gemini Vision API returned no candidates');
      }
      ...
      final modelUsed = data['modelVersion'] as String? ?? model;
      ...
      return VisionResult.failure(
          'Gemini Vision API timed out after 120 seconds');
      ...
      return VisionResult.failure('Gemini Vision API error: $e');
    }
  }
    ...
    try {
      ...
      if (targetsList != null) {
        for (final t in targetsList) {
          try {
            targets
                .add(VisionTarget.fromJson(t as Map<String, dynamic>));
          } catch (_) {
...
/// Riverpod provider for the Gemini Vision service.
final geminiVisionServiceProvider = Provider<GeminiVisionService>((ref) {
  ...
  return GeminiVisionService(connectionStorage: connectionStorage);
});
